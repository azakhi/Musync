...
import {GET_VOTING_STATUS_URL, SERVER_DOMAIN} from "../../config";
...
function createSlots(self){
  ...
  return [0,1,2].map(val => {
    ...
    return (
      <GridListTile onClick={self.handleOnClick}
                    key={val}
                    style={style} >
        <img src={self.props.songs[val].img}
             alt={self.props.songs[val].title} />
             
        <GridListTileBar title={self.props.songs[val].title}
